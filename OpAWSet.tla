...
vars == <<set, seq, abuf, rbuf, incoming, msg, messageset, vc, SECvars>>
vector == [Replica -> Nat]
...
Send(r) == 
    /\ \/ abuf[r] # {}
       \/ rbuf[r] # {}
    /\ Network!RCBroadcast(r, [r |-> r, seq |-> seq[r], update|-> OpUpdate(r), vc |-> [vc EXCEPT ![r][r] = @ + 1][r], abuf|-> abuf[r], rbuf|-> rbuf[r] ])
    /\ abuf' = [abuf EXCEPT ![r] = {}]
    /\ rbuf' = [rbuf EXCEPT ![r] = {}]
    /\ SECSend(r)
    /\ UNCHANGED <<seq, set>>  
...
Receive(r) == 
    /\ Network!RCDeliver(r)
    /\ SECDeliver(r, msg'[r])
    /\ set' = [set EXCEPT ![r] = (@ \cup msg'[r].abuf) \ msg'[r].rbuf]
...
\* Last modified Fri May 31 22:25:24 CST 2019 by xhdn
