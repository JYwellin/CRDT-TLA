...
\@x{}\moduleLeftDash\@xx{ {\MODULE} OpAWSet}\moduleRightDash\@xx{}%
\@x{ {\EXTENDS} AWSet ,\, FiniteSets}%
\@x{ {\CONSTANTS} Read ( \_ ) ,\, InitMsg}%
\@x{}\midbar\@xx{}%
\@x{ {\VARIABLES}}%
\@x{\@s{16.4} aset ,\,\@s{31.63}}%
\@y{\@s{0}%
 \ensuremath{aset[r]}: the set of active elements maintained by \ensuremath{r
 \.{\in} Replica
}}%
\@xx{}%
\@x{\@s{16.4} abuf ,\,\@s{29.44}}%
\@y{\@s{0}%
 \ensuremath{abuf[r]}: the buffer for elements added by \ensuremath{r \.{\in}
 Replica} since the last broadcast
}%
\@xx{}%
\@x{\@s{16.4} rbuf ,\,\@s{30.33}}%
\@y{\@s{0}%
 \ensuremath{rbuf[r]}: the buffer for elements removed by \ensuremath{r
 \.{\in} Replica} since the last broadcast
}%
\@xx{}%
\@x{}%
\@y{\@s{0}%
...
\@xx{}%
\@x{\@s{16.4} incoming ,\,\@s{9.46}}%
\@y{\@s{0}%
 \ensuremath{incoming[r]}: incoming channel at replica \ensuremath{r \.{\in}
 Replica
}}%
\@xx{}%
\@x{\@s{16.4} lmsg ,\,\@s{29.39}}%
\@y{\@s{0}%
 \ensuremath{lmsg[r]}: the last message delivered at \ensuremath{r \.{\in}
 Replica} to the upper-layer protocol
}%
\@xx{}%
\@x{\@s{16.4} vc ,\,\@s{39.94}}%
\@y{\@s{0}%
 \ensuremath{vc[r][s]} denotes the latest message from \ensuremath{s \.{\in}
 Replica} observed by \ensuremath{r \.{\in} Replica
}}%
\@xx{}%
\@x{}%
\@y{\@s{0}%
...
\@xx{}%
\@x{\@s{16.4} doset ,\,\@s{26.52}}%
\@y{\@s{0}%
 \ensuremath{doset[r]}: the set of updates generated by replica \ensuremath{r
 \.{\in} Replica
}}%
\@xx{}%
\@x{\@s{16.4} delset ,\,\@s{24.48}}%
\@y{\@s{0}%
 \ensuremath{uset[r]}: the set of updates delivered by replica \ensuremath{r
 \.{\in} Replica
}}%
\@xx{}%
\@x{\@s{16.4} uincoming ,\,\@s{4.1}}%
\@y{\@s{0}%
 \ensuremath{uincoming[r]}: incoming channel for broadcasting/delivering
 updates at \ensuremath{r \.{\in} Replica
}}%
\@xx{}%
\@x{\@s{16.4} buset\@s{32.89}}%
\@y{\@s{0}%
 \ensuremath{buset[r]}: the buffer of local updates made by \ensuremath{r
 \.{\in} Replica} since the last broadcast
}%
\@xx{}%
\@pvspace{8.0pt}%
\@x{ nVars \.{\defeq} {\langle} incoming ,\, lmsg ,\, vc {\rangle}}%
 \@x{ cVars\@s{1.02} \.{\defeq} {\langle} doset ,\, delset ,\, uincoming ,\,
 buset {\rangle}}%
 \@x{ vars \.{\defeq} {\langle} aset ,\, abuf ,\, rbuf ,\, seq ,\, nVars ,\,
 cVars {\rangle}}%
\@x{}\midbar\@xx{}%
 \@x{ Msg \.{\defeq} [ aid \.{:} Aid ,\, abuf \.{:} {\SUBSET} Element ,\, rbuf
 \.{:} {\SUBSET} Element ]}%
\@x{ Network \.{\defeq} {\INSTANCE} ReliableCausalNetwork}%
\@y{\@s{0}%
 \ensuremath{{\WITH}} \ensuremath{incoming \.{\leftarrow} incoming},
 \ensuremath{lmsg \.{\leftarrow} lmsg}, \ensuremath{vc \.{\leftarrow} vc
}}%
\@xx{}%
\@pvspace{8.0pt}%
 \@x{ ReadOpAWSet ( r ) \.{\defeq} \{ ele . d \.{:} ele \.{\in} aset [ r ]
 \}\@s{8.2}}%
\@y{\@s{0}%
 read the state of \ensuremath{r\.{\in} Replica
}}%
\@xx{}%
\@x{ Correctness \.{\defeq} {\INSTANCE} OpCorrectness}%
\@x{\@s{74.40}}%
\@y{\@s{0}%
 \ensuremath{{\WITH}} \ensuremath{doset \.{\leftarrow} doset},
 \ensuremath{delset \.{\leftarrow} delset}, \ensuremath{uincoming
 \.{\leftarrow} uincoming}, \ensuremath{buset \.{\leftarrow} buset
}}%
\@xx{}%
\@x{}\midbar\@xx{}%
\@x{ TypeOK \.{\defeq}}%
 \@x{\@s{16.4} \.{\land} aset\@s{2.19} \.{\in} [ Replica \.{\rightarrow}
 {\SUBSET} Element ]}%
 \@x{\@s{16.4} \.{\land} abuf \.{\in} [ Replica \.{\rightarrow} {\SUBSET}
 Element ]}%
 \@x{\@s{16.4} \.{\land} rbuf\@s{0.89} \.{\in} [ Replica \.{\rightarrow}
 {\SUBSET} Element ]}%
\@x{\@s{16.4} \.{\land} IntTypeOK}%
\@x{\@s{16.4} \.{\land} Correctness {\bang} CTypeOK}%
\@x{}\midbar\@xx{}%
\@x{ Init \.{\defeq}}%
 \@x{\@s{16.4} \.{\land} aset\@s{2.19} \.{=} [ r \.{\in} Replica \.{\mapsto}
 \{ \} ]}%
\@x{\@s{16.4} \.{\land} abuf \.{=} [ r \.{\in} Replica \.{\mapsto} \{ \} ]}%
 \@x{\@s{16.4} \.{\land} rbuf\@s{0.89} \.{=} [ r \.{\in} Replica \.{\mapsto}
 \{ \} ]}%
\@x{\@s{16.4} \.{\land} IntInit}%
\@x{\@s{16.4} \.{\land} Network {\bang} RCNInit}%
\@x{\@s{16.4} \.{\land} Correctness {\bang} OpCInit}%
\@x{}\midbar\@xx{}%
\@x{ Add ( d ,\, r ) \.{\defeq}\@s{4.1}}%
\@y{\@s{0}%
 \ensuremath{r\.{\in} Replica} adds \ensuremath{d \.{\in} Data
}}%
\@xx{}%
 \@x{\@s{22.06} \.{\land} \.{\LET} e \.{\defeq} [ aid \.{\mapsto} [ r
 \.{\mapsto} r ,\, seq \.{\mapsto} seq [ r ] ] ,\, d \.{\mapsto} d ]}%
 \@x{\@s{33.17} \.{\IN} \.{\land} aset \.{'}\@s{2.19} \.{=} [ aset {\EXCEPT}
 {\bang} [ r ]\@s{2.19} \.{=} @ \.{\cup} \{ e \} ]}%
 \@x{\@s{53.57} \.{\land} abuf \.{'} \.{=} [ abuf {\EXCEPT} {\bang} [ r ]
 \.{=} @ \.{\cup} \{ e \} ]}%
\@x{\@s{22.06} \.{\land} IntDo ( r )}%
\@x{\@s{22.06} \.{\land} Correctness {\bang} OpCDo ( r )}%
\@x{\@s{22.06} \.{\land} {\UNCHANGED} {\langle} rbuf ,\, nVars {\rangle}}%
\@pvspace{8.0pt}%
\@x{ Remove ( d ,\, r ) \.{\defeq}\@s{4.1}}%
\@y{\@s{0}%
 \ensuremath{r\.{\in} Replica} removes \ensuremath{d \.{\in} Data
}}%
\@xx{}%
 \@x{\@s{16.4} \.{\land} \.{\LET} E\@s{4.05} \.{\defeq} \{ ele \.{\in} aset [
 r ] \.{:} ele . d \.{=} d \}\@s{4.1}}%
\@y{\@s{0}%
 \ensuremath{E} may be empty
}%
\@xx{}%
 \@x{\@s{27.51} \.{\IN} \.{\land}\@s{4.1} aset \.{'}\@s{1.29} \.{=} [ aset
 {\EXCEPT} {\bang} [ r ]\@s{1.29} \.{=} @ \.{\,\backslash\,} E ]}%
 \@x{\@s{47.91} \.{\land}\@s{4.10} rbuf \.{'} \.{=} [ rbuf {\EXCEPT} {\bang} [
 r ] \.{=} @ \.{\cup} E ]}%
\@x{\@s{16.4} \.{\land} IntDo ( r )}%
\@x{\@s{16.4} \.{\land} Correctness {\bang} OpCDo ( r )}%
\@x{\@s{16.4} \.{\land} {\UNCHANGED} {\langle} abuf ,\, nVars {\rangle}}%
\@pvspace{8.0pt}%
\@x{ Do ( r ) \.{\defeq}\@s{4.1}}%
\@y{\@s{0}%
 We ignore \ensuremath{ReadOpAWSet(r)} since it does not modify states.
}%
\@xx{}%
 \@x{\@s{22.47} \E\, d \.{\in} Data \.{:} Add ( d ,\, r ) \.{\lor} Remove ( d
 ,\, r )}%
\@x{}\midbar\@xx{}%
\@x{ Send ( r ) \.{\defeq}\@s{4.1}}%
\@y{\@s{0}%
 \ensuremath{r\.{\in} Replica} sends a message
}%
\@xx{}%
\@x{\@s{21.98} \.{\land} buset [ r ] \.{\neq} \{ \}}%
 \@x{\@s{21.98} \.{\land} abuf \.{'} \.{=} [ abuf {\EXCEPT} {\bang} [ r ]
 \.{=} \{ \} ]}%
 \@x{\@s{21.98} \.{\land} rbuf \.{'}\@s{0.89} \.{=} [ rbuf {\EXCEPT} {\bang} [
 r ]\@s{0.89} \.{=} \{ \} ]}%
 \@x{\@s{21.98} \.{\land} Network {\bang} RCNBroadcast ( r ,\, [ aid\@s{5.94}
 \.{\mapsto} [ r \.{\mapsto} r ,\, seq \.{\mapsto} seq [ r ] ] ,\,}%
 \@x{\@s{157.49} abuf \.{\mapsto} abuf [ r ] ,\, rbuf \.{\mapsto} rbuf [ r ] ]
 )}%
\@x{\@s{21.98} \.{\land} IntSend ( r )}%
\@x{\@s{21.98} \.{\land} Correctness {\bang} OpCSend ( r )}%
\@x{\@s{21.98} \.{\land} {\UNCHANGED} {\langle} aset {\rangle}}%
\@pvspace{8.0pt}%
\@x{ Deliver ( r ) \.{\defeq}\@s{4.1}}%
\@y{\@s{0}%
 \ensuremath{r\.{\in} Replica} delivers a message (\ensuremath{lmsg\.{'}[r]})
}%
\@xx{}%
\@x{\@s{16.4} \.{\land} IntDeliver ( r )}%
\@x{\@s{16.4} \.{\land} Network {\bang} RCNDeliver ( r )}%
 \@x{\@s{16.4} \.{\land} Correctness {\bang} OpCDeliver ( r ,\, lmsg \.{'} [ r
 ] . aid )}%
 \@x{\@s{16.4} \.{\land} aset \.{'} \.{=} [ aset {\EXCEPT} {\bang} [ r ] \.{=}
 ( @ \.{\cup} lmsg \.{'} [ r ] . abuf ) \.{\,\backslash\,} lmsg \.{'} [ r ] .
 rbuf ]}%
\@x{\@s{16.4} \.{\land} {\UNCHANGED} {\langle} abuf ,\, rbuf {\rangle}}%
\@x{}\midbar\@xx{}%
 \@x{ Next \.{\defeq} \E\, r \.{\in} Replica \.{:} Do ( r ) \.{\lor} Send ( r
 ) \.{\lor} Deliver ( r )}%
\@pvspace{8.0pt}%
 \@x{ Fairness \.{\defeq} \A\, r \.{\in} Replica \.{:} {\WF}_{ vars} ( Send (
 r ) ) \.{\land} {\WF}_{ vars} ( Deliver ( r ) )}%
\@pvspace{8.0pt}%
...
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 \ensuremath{\.{\,\backslash\,}}* Last modified Sat \ensuremath{Aug} 31
 15:58:24 \ensuremath{CST} 2019 by \ensuremath{xhdn
}%
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 \ensuremath{\.{\,\backslash\,}}* Created \ensuremath{Fri} May 24 14:12:26
 \ensuremath{CST} 2019 by \ensuremath{xhdn
}%
\end{cpar}%
\end{lcom}%
\end{document}
